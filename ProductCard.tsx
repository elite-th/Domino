...
export const ProductCard = forwardRef<HTMLDivElement, ProductCardProps>(({ iceCream, isTarget, animationProgress, index }, ref) => {
    ...
    // Add a little jiggle and pop on landing, starting from full scale for a seamless transition
    ...
    return (
        <motion.div
            ...
            className="group relative rounded-2xl sm:rounded-3xl p-3 sm:p-6 flex flex-col items-center text-center bg-black/20 backdrop-blur-lg border border-amber-200/10 shadow-lg h-full overflow-hidden"
        >
            ...
            <div className="relative z-10 w-full h-full flex flex-col items-center">
                ...
                <div className="flex-grow flex flex-col justify-between w-full">
                    <div>
                        <h3 className="text-lg sm:text-2xl font-bold text-white">{iceCream.name}</h3>
                        <p className="text-amber-200/80 mt-1 sm:mt-2 text-xs sm:text-sm">{iceCream.description}</p>
                    </div>
