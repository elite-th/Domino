...
const Logo = () => (
  <svg width="28" height="28" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label="Ice Cream Logo">
    <path d="M24 4C12.95 4 4 12.95 4 24C4 35.05 12.95 44 24 44C35.05 44 44 35.05 44 24C44 12.95 35.05 4 24 4Z" fill="#0f172a" />
...
const NavLinks = ({ isMobile = false, closeMenu }: { isMobile?: boolean, closeMenu?: () => void }) => {
  ...
  const linkClasses = "px-2.5 py-1 rounded-md text-xs font-medium transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2";
      ...
      {links.map((link, index) => (
        <a
          ...
          className={`
            ...
            ${index === 0 
              ? 'bg-blue-600 text-white font-bold shadow-sm focus:ring-blue-400 focus:ring-offset-white' 
              : 'text-slate-700 hover:bg-slate-100 focus:ring-slate-500 focus:ring-offset-white'}
          `}
        >
          {link}
        </a>
  ...
  return (
    ...
    >
      <div className="container mx-auto max-w-4xl">
        <nav className="relative flex justify-between items-center bg-white/50 backdrop-blur-lg rounded-xl px-3 py-1 md:py-5 border border-slate-200/50 shadow-lg">
          
          {/* Logo group: Sits on the right (start) in RTL */}
          <div className="flex items-center gap-1.5"> 
            <Logo />
            <span className="font-bold text-sm text-slate-800 hidden sm:block">بستنی یخی</span>
          </div>
          ...
          <div>
            ...
            <div className="md:hidden">
              <button onClick={toggleMenu} className="p-2 rounded-md hover:bg-black/10 focus:outline-none focus:ring-2 focus:ring-slate-500" aria-label="Toggle navigation menu" aria-expanded={isOpen}>
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              animate="visible"
              exit="exit"
              className="md:hidden mt-2 bg-white/90 backdrop-blur-xl rounded-xl p-4 border border-slate-200/50 shadow-lg"
            >
